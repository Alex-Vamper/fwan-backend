  ...
  import { dualAuth } from '../middleware/dualAuth.js'; // add this at the top
   ...
   */
  router.patch('/:crateId/thresholds', dualAuth, async (req, res) => {
    ...
    try {
      ...
      const query = req.user
        ? { crateId, userId: req.user.userId }
        : { crateId };

      const crate = await Crate.findOne(query);
   ...
   */
  router.get('/:crateId/thresholds', dualAuth, async (req, res) => {
    try {
      const query = req.user
        ? { crateId: req.params.crateId, userId: req.user.userId }
   ...
   */
  router.get('/:crateId/telemetry', dualAuth, async (req, res) => {
    const { crateId } = req.params;
   ...
   */
  router.patch('/:crateId', dualAuth, async (req, res) => {
    const { crateId } = req.params;
